...
namespace HEAR
{
    ...
    void InnerSysNodelet::onInit(){
        ...
        inner_sys->connect(demux_rot_des->getOutputPort<float>(Demux3::OP::X), ref_sw_roll->getInputPort<float>(InvertedSwitch::IP::NC));
        inner_sys->connect(demux_rot_des->getOutputPort<float>(Demux3::OP::Y), ref_sw_pitch->getInputPort<float>(InvertedSwitch::IP::NC));
        inner_sys->connect(demux_ori->getOutputPort<float>(Demux3::OP::X), hold_ref_roll->getInputPort<float>(HoldVal::IP::INPUT));
        inner_sys->connect(demux_ori->getOutputPort<float>(Demux3::OP::Y), hold_ref_pitch->getInputPort<float>(HoldVal::IP::INPUT));
        inner_sys->connect(hold_ref_roll->getOutputPort<float>(HoldVal::OP::OUTPUT), ref_sw_roll->getInputPort<float>(InvertedSwitch::IP::NO));
        inner_sys->connect(hold_ref_pitch->getOutputPort<float>(HoldVal::OP::OUTPUT), ref_sw_pitch->getInputPort<float>(InvertedSwitch::IP::NO));

        inner_sys->connect(ref_sw_roll->getOutputPort<float>(InvertedSwitch::OP::COM), mux_angle_ref->getInputPort<float>(Mux3::IP::X));
        inner_sys->connect(ref_sw_pitch->getOutputPort<float>(InvertedSwitch::OP::COM), mux_angle_ref->getInputPort<float>(Mux3::IP::Y));
