...
MRFT_Block::MRFT_Block(int b_uid) : Block(BLOCK_ID::MRFT, b_uid) {
    _input_port = createInputPort<float>(IP::INPUT, "INPUT");
    _bias_port = createInputPort<float>(IP::BIAS, "BIAS");
    _output_port = createOutputPort<float>(OP::COMMAND, "COMMAND");
    this->reset();
}
...
void MRFT_Block::process() {
	if(_enable){
		...
		_input_port->read(input);
		_bias_port->read(bias);
		float command = this->mrft_anti_false_switching(input) + bias;
		_output_port->write(command);
	}    
}
